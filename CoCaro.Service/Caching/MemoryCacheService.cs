...
namespace CoCaro.Service.Caching
{
    ...
    /// </summary>
    public partial class MemoryCacheService : ILocker, ICacheService
    {
        #region Fields
        ...
        public MemoryCacheService(IMemoryCache memoryCache,ICacheKeyService cacheKeyService)
        {
        ...
        public T Get<T>(CacheKey key, Func<T> acquire = null)
        {
            if (key.CacheTime <= 0)
                return acquire();
        ...
        public T Get<T>(CacheKey key)
        {
            ...
            var result = _memoryCache.Get<T>(key.Key);
        ...
        public CacheKey CreateCacheKey(CacheKey Key, params object[] keyObjects)
        {
            return _CacheKeyService.PrepareKey(Key, keyObjects);
        }

        public CacheKey CreateCacheKey(string Key, params object[] keyObjects)
        {
            return _CacheKeyService.PrepareKey(new CacheKey(Key), keyObjects);
        }
